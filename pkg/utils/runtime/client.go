...
// WithTLS enables automatic HTTPS/mTLS addressing of the agent-runtime. Unlike
// the manual WithBaseURL + WithHTTPClient pair, it needs no hand-built URL or
// http.Client: the client dials the sandbox Pod IP on RuntimeTLSPort while
// verifying the server certificate against RuntimeServerSNI (overridable via
...
// VerifyClientCertIfGiven). An invalid material is reported by the first call.
func WithTLS(m TLSMaterial) Option {
	return func(rc *runtimeClient) {
		rc.tlsEnabled = true
		rc.tlsMaterial = &m
	}
...
func NewRuntime(sbx *agentsv1alpha1.Sandbox, opts ...Option) Runtime {
	...
	// invalid material surfaces as a permanent (non-retried) error on first call.
	if rc.tlsEnabled {
		if rc.authority == "" {
		...
		if rc.tlsMaterial != nil {
			rc.tlsClientConfig, rc.tlsConfigErr = buildClientTLSConfig(*rc.tlsMaterial, rc.authority)
		} else {
			rc.tlsConfigErr = fmt.Errorf("WithTLS requires TLS material")
		}
	}
	return rc
}
...
// GetRuntimeURL. In TLS mode (see WithTLS) it returns the HTTPS authority URL
...
// sandbox Pod IP separately by dialIPFor. It returns an empty string when the
// runtime is not yet addressable; callers must treat that as "not ready" (a
// retryable condition).
...
// request URL host (plain-HTTP mode or an explicit WithBaseURL override), in
// which case the standard http.Client dials by URL host.
func (r *runtimeClient) dialIPFor(sbx *agentsv1alpha1.Sandbox) string {
	if !r.tlsEnabled || r.baseURLOverride != "" || sbx == nil {
		return ""
	}
...
func (r *runtimeClient) call(ctx context.Context, method, path string, reqBody, respOut any) error {
	...
	do := func() error {
		...
		// between retries. Otherwise use the shared client (plain HTTP, or a
		// caller-provided client via WithHTTPClient/WithBaseURL).
		httpClient := r.client
